...
function myarcade_notices() {
	...
	if ( myarcade_schluessel() === false ) {
		...
		?>
		<form method="post" name="myabp_lk" <?php echo $action; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped ?> style="background-color: #FEEDE9;border: 1px solid #DD897F;margin: 10px;padding: 5px 5px 5px 10px; text-align: left; border-radius:5px; -moz-border-radius:5px;-webkit-border-radius:5px;">
			<div style="margin:5px 0">
				<?php
				if ( myarcade_license_status() === -1 ) {
					esc_html_e( 'MyArcadePlugin license server could not be reached! Check your outbound connection!', 'myarcadeplugin' );
					?>
					<br /><br />
					<?php
					esc_html_e( 'Please try again later...', 'myarcadeplugin' );
					?>
					<br/><br/>
					<?php
				}

				esc_html_e( 'MyArcadePlugin needs to be activated. Please enter your personal license key.', 'myarcadeplugin' );
				?>
			</div>

			<input type="text" size="40"  name="myarcade_schluessel" /> <input id="submit" type="submit" name="submit" value="Activate Plugin" />
			<div style="margin:5px 0">
				<a href="https://myarcadeplugin.com/buy/" target="_blank"><?php esc_html_e( 'Buy a MyArcadePlugin license &rarr;', 'myarcadeplugin' ); ?></a>
			</div>
		</form><br />
		<?php
	}
...
function myarcade_pruefe_schluessel() {
	...
	$schluessel = filter_input( INPUT_POST, 'myarcade_schluessel' );
	...
	if ( myarcade_get_license_data( $schluessel ) ) {
